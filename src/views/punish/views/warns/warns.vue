<template>
  <div class="table-container">
    <table v-if="warns">
      ...
      <tbody>
        <tr v-for="warn in warns" :key="warn.id" @click="openPage(warn.id)">
          <td>{{ warn.id }}</td>
          <td>
            <p>
...
const warns = ref<WarnData[]>([]);

const fetchData = async () => {
  try {
    const response = await axios.get(`${import.meta.env.VITE_API_URL}/punish/warns`);
